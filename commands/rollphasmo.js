...
module.exports = {
	...
	async execute(interaction) {
		const buttons = new ActionRowBuilder().addComponents(
			...
			new ButtonBuilder().setCustomId('finished').setLabel('Finished').setStyle(ButtonStyle.Success)
		);

		const pullButton = new ActionRowBuilder().addComponents(
			new ButtonBuilder().setCustomId('takeOne').setLabel('Take One').setStyle(ButtonStyle.Success),
			new ButtonBuilder().setCustomId('takeAll').setLabel('Take All').setStyle(ButtonStyle.Primary)
		);
		...
		const embededMessage = new EmbedBuilder()
			...
			.addFields({ name: 'Rolled Items', value: '\u200B' })
		...
		await interaction.reply({ content: 'Roulette Started!', ephemeral: true });
		...
		collector.on('collect', async (ButtonInteraction) => {
			...
			switch (id) {
				case 'roll':
					...
					embededMessage.addFields({ name: 'You rolled: ', value: randomItem });
